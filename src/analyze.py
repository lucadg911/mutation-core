...
def analyze(folder_path, command="", jobs=0, timeout=10000, survival_threshold=0.3, time_threshold=0.5):
    """
    ...
    Args:
        ...
        time_threshold: When to check survival rate (0.5 = halfway)
    """
    killed = []
    not_killed = []
    ...
    try:
        ...
        time_threshold_seconds = timeout * time_threshold

        for i, file_name in enumerate(files, 1):
            current_time = time.time() - start_time

            # Check if we've reached the time threshold
            if current_time >= time_threshold_seconds:
                current_survival_rate = len(not_killed) / (len(killed) + len(not_killed))
                if current_survival_rate > survival_threshold:
                    print(f"\nTerminating early: {current_survival_rate:.2%} mutants surviving after {current_time:.1f}s")
        ...
        score = len(killed) / (len(killed) + len(not_killed)) if killed or not_killed else 0
